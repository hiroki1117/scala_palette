...
object TasksRoutes:
  ...
  def routes[F[_]: Async]: HttpRoutes[F] =
    ...
    val getTaskRoute = TasksEndpoint.getTask.serverLogic[F] {
      case (userId, id) =>
    ...
    val createTaskRoute = TasksEndpoint.createTask.serverLogic[F] {
      case (userId, req) =>
    ...
    val updateTaskRoute = TasksEndpoint.updateTask.serverLogic[F] {
      case (userId, id, req) =>
        // DTO -> domain 変換してビジネスロジック実行
    ...
    val deleteTaskRoute = TasksEndpoint.deleteTask.serverLogic[F] {
      case (userId, id) =>
